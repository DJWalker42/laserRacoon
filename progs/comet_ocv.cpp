...
int main()
{
	...
	velocity[0] = 8.79e2 * phys::yrs * 1.e-4 / phys::AU;   //speed of comet at aphelion in AU per year
	velocity[1] = 0.;

	phys::state initial(t, position, velocity);

	phys::Gravity grav;
	phys::RKF45 rkf45(&grav, initial, step);

	phys::ODEStorage data = rkf45.fullSolve(300.);

	{
		phys::Viewer viewer;
		viewer.plot(data, phys::Viewer::XY);
	}
