...
RootSearch::RootSearch() :
		m_function(), m_derivative(), m_lft_brc(), m_rht_brc() {
	initialise_consts();
}

RootSearch::RootSearch(double (*func)(double)) :
		m_function(func) {
	initialise_consts();
}
...
RootSearch::RootSearch(double (*func)(double), double (*deriv)(double),
		...
		m_function(func), m_derivative(deriv), m_lft_brc(initial_guess), m_rht_brc(
				initial_guess) {
	initialise_consts();
}
...
void RootSearch::set_brackets(double new_lft_brc, double new_rht_brc) {
	...
	if (m_is_bisec)
		check_root_is_bracketed(__FUNCTION__, m_function(m_lft_brc),
				m_function(m_rht_brc));
...
void RootSearch::check_all(std::string call_func) {
	...
	if (m_is_bisec)
		check_root_is_bracketed(call_func, m_function(m_lft_brc),
				m_function(m_rht_brc));
}
...
void RootSearch::check_root_is_bracketed(const std::string &call_func,
		double func_lft, double func_rht) {
...
double Hybrid_B_N_R::find_root(bool print_output) {
	...
	size_t iter_cnt = 0;
	while (m_error > m_tolerance && iter_cnt++ < m_iter_max) {
		if ((DFR * (root - m_lft_brc) - FR) * (DFR * (root - m_rht_brc) - FR)
				<= 0) {
			delta = -FR / DFR;
			root += delta;
		} else {
