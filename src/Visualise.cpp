...
const static cv::Scalar colour[] = { cv::Scalar(255, 0, 0),		//blue
...
cv::Point compute_origin(const cv::Rect &plot, const axis_props &x_axis,
...
coords compute_data_1(const stdVec_d &x_vals, const stdVec_d &y_vals,
		...
		double m_scale_x, double m_scale_y) {
	...
	phys::data plot_data(x_vals, y_vals);
	bool sort = true;

	phys::Linear linear_interp(plot_data, sort);
	...
	for (uint i = 0; i < x_interp.size(); ++i) {
		int y_temp = origin.y
				- int(round(m_scale_y * (y_interp[i] - origin_val_y)));
...
coords compute_data_2(const stdVec_d &x_vals, const stdVec_d &y_vals,
		const cv::Point &origin, //pixel location of axis origin
...
void Viewer::plot(const phys::ODEStorage &data, graph_choice choice,
		uint dim1, uint dim2) {
	if (data.get_independent().size() < 2) {
		throw std::runtime_error(
				"Not enough data to make a plot - check your code\n");
	}
...
void Viewer::plot(const phys::Storage<double> &data, bool splitAxes) {
	m_x_axis.title = data.get_x_name();
...
void Viewer::plot_split(const stdVec_d &x, const std::vector<stdVec_d> &y) {
	...
	if (m_g_choice < 2)
		data = compute_data_1(x, y[1], m_axis_origin, data_origin,
				m_x_axis.scale, m_y_axis2.scale);
	else
		data = compute_data_2(x, y[1], m_axis_origin, data_origin,
				m_x_axis.scale, m_y_axis2.scale);

	if (m_using_range_x || m_using_range_y)
		chk_data(data);
...
void Viewer::plot(const stdVec_d &x, const stdVec_d &y) {
	...
	if (m_g_choice < 2)
		data = compute_data_1(x, y, m_axis_origin, data_origin, m_x_axis.scale,
				m_y_axis.scale);
	else
		data = compute_data_2(x, y, m_axis_origin, data_origin, m_x_axis.scale,
				m_y_axis.scale);
...
void Viewer::plot(const stdVec_d &x, const std::vector<stdVec_d> &y) {
	...
	for (int i = 0; i < y.size(); ++i) {
		...
		if (m_g_choice < 2)
			data = compute_data_1(x, y[i], m_axis_origin, data_origin,
					m_x_axis.scale, m_y_axis.scale);
		else
			data = compute_data_2(x, y[i], m_axis_origin, data_origin,
					m_x_axis.scale, m_y_axis.scale);
...
void Viewer::draw_key(uint which, const cv::Scalar &colour) {
	if (m_key_titles.empty())
		return; //Do nothing.
	int offset = m_rows / 25;
	//compute location of where to put key.
	cv::Point key_loc = cv::Point(m_cols - m_cols / 10,
			m_rows / 10 + offset * which);
	//draw it
	cv::putText(m_BG, m_key_titles[which], key_loc, CV_FONT_HERSHEY_PLAIN,
			m_font_scale * 1.2, colour);
}
		...
		for (ccoords_it it = data.begin(); it != data.end() - 1; ++it) {
			if (it->first > 0 && (it + 1)->first > 0)
				cv::line(m_BG, cv::Point(it->first, it->second),
						cv::Point((it + 1)->first, (it + 1)->second),
						m_dt_colour);
		}
	}
...
void Viewer::plot_ani_impl(const coords &data, int delay) {
	cv::putText(m_BG, "Press Esc to continue...", cv::Point(10, 10),
			CV_FONT_HERSHEY_PLAIN, m_font_scale * 0.7, m_dt_colour);
...
void Viewer::drawXaxis(int xo) {
	...
	cv::Point x_pos(int(m_cols / 2),
			m_plot_area.y + m_plot_area.height
					+ int(m_rows * m_offset_y / 150));
	//... and draw it
	cv::putText(m_BG, m_x_axis.title, x_pos, CV_FONT_HERSHEY_PLAIN,
			1.2 * m_font_scale, m_dt_colour);
}
...
void Viewer::drawYaxis(int yo, bool onLeft) {
	...
	int title_x =
			onLeft ?
					int(m_cols * m_offset_x / 100 / 2) :
					m_plot_area.x + m_plot_area.width
							+ int(m_cols * m_offset_x / 100 / 2);

	cv::Point y_pos(title_x, int(m_rows * m_offset_y / 100 / 2));
	cv::putText(m_BG, onLeft ? m_y_axis.title : m_y_axis2.title, y_pos,
			CV_FONT_HERSHEY_PLAIN, 1.2 * m_font_scale, m_dt_colour);
}
...
void Viewer::draw_labels(axis AXIS, double val_o, double val_s, double scale,
	...
	case X:
		...
		do {
			...
			cv::putText(m_BG, format(disp_val, 3, 1.e-2, 1.e4), label_loc,
					CV_FONT_HERSHEY_PLAIN, .8 * m_font_scale, label_clr);
			tick_loc += tick_space;
			disp_val += val_s;
...
void Viewer::add_data(const phys::ODEStorage &data, uint dim1,
		uint dim2) {
	switch (m_g_choice) {
	case DEPEN:
...
void Viewer::add_data(const phys::Storage<double> &data,
		bool is_data_singular) {
	add_data(data.get_independent(), data.get_dependent(), is_data_singular);
}

void Viewer::add_data(const stdVec_d &x, const std::vector<stdVec_d> &y,
		bool is_data_singular) {
	for (uint i = 0; i < y.size(); ++i) {
...
void Viewer::add_data(const stdVec_d &x, const stdVec_d &y,
		bool is_data_singular) {
	if (!m_is_drawn)
...
void Viewer::chk_data(coords &data) {
	...
	for (coords_it it = data.begin(); it != data.end(); ++it) {
		...
		if (it->first >= m_plot_area.x
				&& it->first <= m_plot_area.x + m_plot_area.width) {
			++count;
			if (count == 1)
				lft_bound = it; //first value within x-bounds
			//if x in bounds check to see if y out of bounds.
			if (it->second < m_plot_area.y
					|| it->second > m_plot_area.y + m_plot_area.height) {
				it->first = -10;
	...
	if (m_g_choice < 2 && rht_bound != data.begin()
			&& lft_bound != data.begin()) {
...
void Square::draw(Viewer *v, const cv::Point &loc) {
	cv::Rect r_loc(loc.x - m_hull_x / 2, loc.y - m_hull_x / 2, m_hull_x,
			m_hull_x);
