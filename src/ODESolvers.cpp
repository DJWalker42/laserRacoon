...
ODESolver::ODESolver(Diff_eqn *p_diff, const state &system,
		double step) :
		m_ptr_diff_eqn(p_diff), m_current(system), m_step(step) {
	initialise_solver();
}

void ODESolver::set_system(const state &system, Diff_eqn *p_diff,
		double step) {
	m_current = system;
	if (p_diff != 0)
...
stdVec_d ODESolver::deriv_B(double x, const stdVec_d &y) {
	for (int i = m_mid_idx; i < m_num_of_vars; ++i)
		m_deriv_result[i] = m_ptr_diff_eqn->differential_function(x, y, m_dims,
				i - m_mid_idx);
...
size_t ODESolver::num_steps(double start, double &end) {
	...
	if (N - tN < 1.e-6)
		return size_t(N); //the number of steps is close enough to an int value
	else {
...
ODEStorage Euler::fullSolve(double end) {
	size_t N = num_steps(m_current.x, end), count = 0;
	ODEStorage container(m_current);
	while (count++ < N) {
...
ODEStorage Imp_Euler::fullSolve(double end) {
	size_t N = num_steps(m_current.x, end), count = 0;
	ODEStorage container(m_current);
	state next = m_current;
	while (count++ < N) {
...
ODEStorage Mod_Euler::fullSolve(double end) {
	size_t N = num_steps(m_current.x, end), count = 0;
	ODEStorage container(m_current);
	state next = m_current;
	while (count++ < N) {
		...
		m_k[1] = deriv(next.x + m_step, next.y);
		for (size_t i = 0; i < m_num_of_vars; ++i)
			next.y[i] = m_current.y[i]
					+ (m_step / 2.) * (m_k[0][i] + m_k[1][i]);
		next.x = m_current.x + m_step;
		m_current = next;
	}
	container.store(m_current);
...
ODEStorage RK4::fullSolve(double end) {
	size_t N = num_steps(m_current.x, end), count = 0;
	ODEStorage container(m_current);
	state next = m_current;
	while (count++ < N) {
...
void Stormer_Verlet::init_method() {
	...
	for (size_t i = 0; i < m_dims; i++)
		m_current.y[i] += m_step * m_k[0][i]
				+ (m_step * m_step / 2.) * m_k[0][i + m_dims];
	m_current.x += m_step;
}
...
state Stormer_Verlet::solve() {
	...
	for (size_t i = 0; i < m_dims; i++)
		next.y[i] = 2. * m_current.y[i] - m_previous.y[i]
				+ (m_step * m_step) * m_k[0][i + m_dims];
	next.x = m_current.x + m_step;
...
ODEStorage Stormer_Verlet::fullSolve(double end) {
	size_t N = num_steps(m_current.x, end), count = 0;
	ODEStorage container(m_current);
...
state Leapfrog::solve() {
	...
	for (size_t i = 0; i < m_dims; i++)
		next.y[i + m_dims] = m_current.y[i + m_dims]
				+ m_step * m_k[0][i + m_dims];
	next.x = m_current.x + m_step;
	m_current = next;
	return next;
}
...
ODEStorage Leapfrog::fullSolve(double end) {
	size_t N = num_steps(m_current.x, end), count = 0;
	ODEStorage container(m_current);
	state next = m_current;
	while (count++ < N) {
...
ODEStorage Leapfrog::sync(const ODEStorage &data) {
	...
	ODEStorage retval(state(inde[0], depn[0], deri[0]));
	size_t n = inde.size();
...
state Velocity_Verlet::solve() {
	...
	for (size_t i = 0; i < m_dims; ++i)
		next.y[i + m_dims] = m_current.y[i + m_dims]
				+ (m_step / 2.) * (m_k[0][i + m_dims] + m_k[1][i + m_dims]);
	next.x = m_current.x + m_step;
	m_current = next;
	return next;
}

ODEStorage Velocity_Verlet::fullSolve(double end) {
	size_t N = num_steps(m_current.x, end), count = 0;
	ODEStorage container(m_current);
	state next = m_current;
	while (count++ < N) {
...
state RKF45::solve() {
	...
	while (next.x == m_current.x) //i.e. integrator not advanced
	{
		...
		for (size_t i = 0; i < m_num_of_vars; ++i)
			next.y[i] = m_current.y[i]
					+ m_step * (b20 * m_k[0][i] + b21 * m_k[1][i]);

		m_k[2] = deriv(next.x + a2 * m_step, next.y);
		for (size_t i = 0; i < m_num_of_vars; ++i)
			next.y[i] = m_current.y[i]
					+ m_step
							* (b30 * m_k[0][i] + b31 * m_k[1][i]
									+ b32 * m_k[2][i]);
		...
		for (size_t i = 0; i < m_num_of_vars; ++i)
			next.y[i] = m_current.y[i]
					+ m_step
							* (b40 * m_k[0][i] + b41 * m_k[1][i]
									+ b42 * m_k[2][i] + b43 * m_k[3][i]);

		m_k[4] = deriv(next.x + a4 * m_step, next.y);
		for (size_t i = 0; i < m_num_of_vars; ++i)
			next.y[i] = m_current.y[i]
					+ m_step
							* (b50 * m_k[0][i] + b51 * m_k[1][i]
									+ b52 * m_k[2][i] + b53 * m_k[3][i]
									+ b54 * m_k[4][i]);
		...
		double bigerr = 0.0;
		for (size_t i = 0; i < m_num_of_vars; ++i) {
			yhat[i] = m_current.y[i]
					+ m_step
							* (c0 * m_k[0][i] + c2 * m_k[2][i] + c3 * m_k[3][i]
									+ c4 * m_k[4][i] + c5 * m_k[5][i]);
			double err = m_step
					* fabs(
							d0 * m_k[0][i] + d2 * m_k[2][i] + d3 * m_k[3][i]
									+ d4 * m_k[4][i] + d5 * m_k[5][i]);
			//our estimate for the new step should be based on the largest error estimate
			if (fabs(err) > fabs(bigerr))
				bigerr = err;
		}
		hnew = alpha * m_step * sqrt(sqrt(fabs(m_step) * tol / bigerr));
		if (fabs(hnew) > 4.0e0 * fabs(m_step))
			hnew = 4.0e0 * m_step;
		if (fabs(hnew) < 1.e-1 * fabs(m_step))
			hnew = 1.e-1 * m_step;
		...
		} else // accept the integration at m_current step size
		{
			...
			if (m_targetState == TARGET
					&& (sign * (next.x + m_step) >= sign * m_nextTarget)) {
...
ODEStorage RKF45::fullSolve(double end) {
	ODEStorage container(m_current);
	state next = m_current;
	yhat = m_current.y;
	while (sign * next.x < sign * end) {
		...
		while (next.x == m_current.x) //i.e. integrator not advanced
		{
			...
			for (size_t i = 0; i < m_num_of_vars; ++i) {
				yhat[i] = m_current.y[i]
						+ m_step
								* (c0 * m_k[0][i] + c2 * m_k[2][i]
										+ c3 * m_k[3][i] + c4 * m_k[4][i]
										+ c5 * m_k[5][i]);
				double err = m_step
						* fabs(
								d0 * m_k[0][i] + d2 * m_k[2][i] + d3 * m_k[3][i]
...
ODEStorage RKF45::fullSolveWrapped(double end) {
	ODEStorage container(m_current);
	do {
		this->wrap();
 ...
 */
ODEStorage Numerov::fullSolve(double end) {
	size_t N = num_steps(m_current.x, end), count = 0;
	state temp(0.0, 0.0); //arbitrary double values.
	ODEStorage container(temp); //this just sets up the storage, it doesn't store data.
	state next = m_current;
	while (count++ < N) {
		container.store(m_current);
		next.x += m_step;
...
void Numerov::__error_order() const {
	std::string errmsg =
			"The Numerov method is specifically used for 2nd order differential\n";
	errmsg +=
			"equations of the form: u''(x) + q(x)u(x) = 0. Please check your code.";
	throw(std::runtime_error(errmsg));
}
void Numerov::__error_dimensions() const {
	std::string errmsg =
			"The Numerov method is use in the one dimensional case only\n";
...
ODEStorage Numerov_s::fullSolve(double end) {
	size_t N = num_steps(m_current.x, end), count = 0;
	ODEStorage container(m_current);
	state next = m_current;
	while (count++ < N) {
...
void Numerov_s::__error_order() const {
	std::string errmsg =
			"The Numerov_s method is specifically used for 2nd order differential\n";
	errmsg +=
			"equations of the form: u''(x) + q(x)u(x) = s(x). Please check your code.";
	throw(std::runtime_error(errmsg));
}
void Numerov_s::__error_dimensions() const {
	std::string errmsg =
			"The Numerov_s method is use in the one dimensional case only\n";
	errmsg += "Please check your code.";
	throw(std::runtime_error(errmsg));
}
