	...
	namespace storage{
		...
		*/
		...
		public: //interface functions
			...
			*/
			void set(const phys::ode::state & system);
			...
			*/
			void set_x_name(const std::string& name)	{ m_xName = name; }
			void set_y_name(const std::string& name)	{ m_yName = name; }
			void set_dy_name(const std::string& name)	{ m_dyName = name; }

			/*	Functions to get data names	*/
			std::string get_x_name() const { return m_xName; }
			std::string get_y_name() const { return m_yName; }
			std::string get_dy_name() const { return m_dyName; }

			/*Functions to get size information. */
			size_t num_grid_points() const { return m_independent.size(); }
			size_t num_of_vars() const { return m_dependent.size(); } // should equal order * dims * num_grid_points
		...
		public: //interface
			...
			inline void copy(const std::vector<T>& x_vals, const std::vector<T>& y_vals);
			//stores a vector of the independent value, and multiple vectors of dependent variables. Clears any dependent values.
			inline void copy(const std::vector<T>& x_vals, const std::vector<std::vector<T>>& y_vals);
			...
			void set_x_name(const std::string& name){ m_xTitle = name; }
			void set_key_names(const std::vector<std::string>& names) { m_keyTitles = names; }
			void set_y_name(const std::string& name){ m_yTitle = name; }
			void add_to_key(const std::string& name) { m_keyTitles.push_back(name); }
			...
			*/
			...
			//Query data
			std::vector<T> get_independent() const		{ return m_independent; }
			...
			std::vector<std::vector<T>> get_multi() const		{ return m_multi; }
			std::vector<T> get_dependent(size_t dim) const
			{
				if (m_multi.empty()) throw(std::runtime_error("No multi-data stored"));
				if (dim > m_multi.size() - 1) throw(std::runtime_error("Dimension too large"));
				return m_multi[dim];
			}
			//Query names
			std::string get_x_name() const { return m_xTitle; }
			std::string get_y_name() const { return m_yTitle; }
			std::string get_name(size_t index) const
			{
				assert(index < m_keyTitles.size());
				return m_keyTitles[index];
			}
			std::vector<std::string> get_key_names() const { return m_keyTitles; }
		private: //functions
