...
namespace phys{
	namespace storage{
		...
		Storage<T>::Storage(const std::string& x_name,
			...
			m_independent(),
			m_dependent(),
			m_multi(key_names.size()),
			m_xTitle(x_name),
			m_yTitle(),
			m_keyTitles(key_names)
		{}
		...
		template<typename T> inline
			void Storage<T>::copy(const std::vector<T>& x_vals, const std::vector<T>& y_vals)
		{
		...
		template<typename T> inline
			void Storage<T>::copy(const std::vector<T>& x_vals, const std::vector<std::vector<T>>& y_vals)
		{
			m_dependent.clear();
			m_independent = x_vals;
			m_multi = y_vals;
		...
		bool Storage<T>::read(const std::string& filename)
		{
			...
			if (cols == 1){ std::cout << "No m_dependent variable\n"; return false; }
		...
		{
			if (m_multi.empty()){
				if (write_single(filename, headers)) return;
			}
			else{
				if (write_multi(filename, headers)) return;
			}
			//If here then there has been an error opening a file for writing
			// -- display warning but don't throw error.
			std::cout << "Warning: " << filename << " could not be opened for writing" << std::endl;
			return;
		}
		...
		bool Storage<T>::write_single(const std::string& fn, bool hd) const
		{
			...
			if (hd) output << "#" << m_xTitle << "\t" << m_yTitle << "\n";
