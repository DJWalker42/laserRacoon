...
namespace phys{
	 ...
	 */
		...
		protected:
			...
			RootSearch(double (*func)(double), double (*deriv)(double), double left, double right);
		public:
			/*	Destructor */
			virtual ~RootSearch(){}
		private:
		...
		public:
			...
			*/
			...
			void set_function(double(*func)(double)){ m_function = func; }
			...
			double lft_bracket() const { return m_lft_brc; }
			//Query the right bracket
			double rht_bracket() const { return m_rht_brc; }
		...
		private:
			...
			void check_root_is_bracketed(	const std::string& call_func,
											double func_lft,
											double func_rht );
		protected:
		...
		public:
			Bisection() : RootSearch() 
			{}
			Bisection( double (*func)(double) ): RootSearch(func) 
			{}
		...
		public:
			Secant() : RootSearch()
			{m_is_bisec = false;}
			Secant(double (*func)(double)) : RootSearch(func) 
			{m_is_bisec = false;}
		...
		public:
			...
			Newton_Raphson(double (*func)(double), double (*deriv)(double), double first_guess) :
		...
		public:
			Hybrid_B_N_R() :  RootSearch() 
			{m_is_newton = true;}
			Hybrid_B_N_R(double (*func)(double), double (*deriv)(double)) : 
				RootSearch(func, deriv) 
			{m_is_newton = true;}
			Hybrid_B_N_R(double (*func)(double), double (*deriv)(double), double left, double right) : 
		...
		public:
			Hybrid_B_S() : RootSearch() 
			{}
			Hybrid_B_S(double (*func)(double)) : RootSearch(func)
			{}
