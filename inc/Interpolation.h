...
struct data {
	...
	void set_x_values(const stdVec_d &x_to_set) {
		x = x_to_set;
	}
	void set_y_values(const stdVec_d &y_to_set) {
		y = y_to_set;
	}

	void set_values(const stdVec_d &x_to_set, const stdVec_d &y_to_set) {
		x = x_to_set;
		y = y_to_set;
	}
...
protected:
	...
	Interpolator(const stdVec_d &x_vals, const stdVec_d &y_vals, bool sort =
			true, double tolx = DBL_EPSILON);

	Interpolator(const data &d, bool sort = true, double tolx = DBL_EPSILON);
public:
...
public:
	...
	Lagrange(data d, bool sort = true) :
			Interpolator(d, sort) {
	}
	double interpolate(double x);
};
...
public:
	Cubic() :
			Interpolator(), m_x(), m_y(), m_a(), m_b(), m_c(), m_d() {
	}
	Cubic(const data &d) :
			Interpolator(), m_x(), m_y(), m_a(), m_b(), m_c(), m_d() {
