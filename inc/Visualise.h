...
namespace phys{
	namespace visual{
		...
		enum shape_choice { CIRCLE, SQUARE, TRIANGLE, CROSS, ARROW };
		...
		public:
			Viewer() : m_rows(450),
				...
				m_x_range(0.0, 0.0),
				m_y_range(0.0, 0.0),
				m_using_range_x(false),
				m_using_range_y(false),
				m_plot_area(),
				m_axis_origin(cv::Point(m_rows / 2, m_cols / 2)),
				m_bg_colour(cv::Scalar(0, 0, 0)),
				m_ax_colour(cv::Scalar(255, 0, 0)),
				m_dt_colour(cv::Scalar(0, 255, 0)),
				m_clr_counter(1), //default to green
				m_val_origin_x(0.0),
				m_val_origin_y(0.0),
				m_key_titles(),
				m_plot_name("Plot"),
			...
			Viewer(const int width,
				const int height,
				const cv::Scalar& bg_col = cv::Scalar(0, 0, 0),
			...
			void add_data(const stdVec_d& x, const stdVec_d& y, bool is_data_singular = true);

			void add_data(const stdVec_d& x, const std::vector<stdVec_d>& y, bool is_data_singular = true);

			/* Specify a path and filname including .extension (e.g. .png, .jpg, .bmp) */
			void save(const std::string& filename) const { cv::imwrite(filename, m_BG); }
			...
			void set_resolution(int num_cols, int num_rows, const cv::Scalar& bg_clr = cv::Scalar())
			{
				m_rows = num_rows;
				m_cols = num_cols;
				m_font_scale = m_rows*m_cols / 1000 / 1000;
			...
			void set_x_name(const std::string& name) { m_x_axis.title = name; }
			void set_y_name(const std::string& name) { m_y_axis.title = name; }
			void set_y2_name(const std::string& name) { m_y_axis2.title = name; }
			void set_key_name(const std::vector<std::string>& names) { m_key_titles = names; }
			void set_plot_name(const std::string& name) { m_plot_name = name; }

			void set_x_origin(double value){ m_val_origin_x = value; }
			void set_y_origin(double value){ m_val_origin_y = value; }

			void set_pause(int new_pause){ m_pause = new_pause; }
		...
		private:
			...
			void draw_labels(axis which,
				double value_origin,
				double value_spacing,
				double scale,
				const cv::Point& m_axis_origin);
		...
		void Viewer::processAxis(const std::vector<T>& values, axis which)
		{
			...
			case X:
				...
				if (!m_using_range_x){
					phys::maths::minMax(values, minVal, maxVal);
				}
				else{
				...
				if (minScale == maxScale) {
					throw std::runtime_error("Problem with data: have you passed in a vector of constant values?\n"); 
				}
			...
			T &maxVal)
		{
			...
			phys::maths::minMax(min, minVal, temp);
			phys::maths::minMax(max, temp, maxVal);
		}
