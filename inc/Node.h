...
public:
	...
	Node(double xCoord = 0.) :
			m_Xcoord(xCoord), m_Ycoord(0.), m_globalIndex(0) {
	}
	/* Constructor for a Node in 2D space */
	Node(double xCoord, double yCoord, int globalIdx) :
			m_Xcoord(xCoord), m_Ycoord(yCoord), m_globalIndex(globalIdx) {
	}
...
class InteriorNode {
public:
	InteriorNode(double xCoord, double yCoord = 0., int globalIdx = 0,
			int northIdx = 0, int eastIdx = 0, int southIdx = 0,
			int westIdx = 0) :
			m_Xcoord(xCoord), m_Ycoord(yCoord), m_globalIdx(globalIdx) {
...
public:
	...
	int northIdx() const noexcept {
		return m_neighbour[NORTH];
	}
	int southIdx() const noexcept {
		return m_neighbour[SOUTH];
	}
	int eastIdx() const noexcept {
		return m_neighbour[EAST];
	}
	int westIdx() const noexcept {
		return m_neighbour[WEST];
	}
