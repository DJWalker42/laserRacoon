...
template<class T, uint32_t N>
staticVec<T, N>::staticVec(const T &a, const T &b) {
	std::cout << "staticVec double 2 constructor (" << a << "," << b << ")"
			<< std::endl;
	m_element[0] = a;
	m_element[1] = b;
}
...
template<class T, uint32_t N>
staticVec<T, N>::staticVec(const T &a, const T &b, const T &c) {
	std::cout << "staticVec double 3 constructor (" << a << "," << b << "," << c
			<< ")" << std::endl;
	m_element[0] = a;
	m_element[1] = b;
	m_element[2] = c;
 ...
 **********************************************************/
template<class T, uint32_t N>
const T& staticVec<T, N>::operator[](uint32_t i) const {
	return m_element[i];
} // read ith m_element of the vector

template<class T, uint32_t N>
T& staticVec<T, N>::operator[](uint32_t i) {
	return m_element[i];
} // write ith m_element
