...
public:
	...
	staticMat(const staticVec<T, N> &v, bool t = false);

	/* Special constructor for a staticMat with two rows (M) (v,u) */
	staticMat(const staticVec<T, N> &v, const staticVec<T, N> &u,
			bool t = false);

	/* Special constructor for a staticMat with three rows (v,u,w) */
	staticMat(const staticVec<T, N> &v, const staticVec<T, N> &u,
			const staticVec<T, N> &w, bool t = false);
	...
	staticMat(const T &val, bool t = false);

	/*	Constructor from base class object */
	staticMat(const staticVec<staticVec<T, N>, M> &vv, bool t = false);
...
//type definition for 2 x 2 matrices
using staticMat2 = staticMat<double, 2, 2>;
//type definition for 3 x 3 matrices
using staticMat3 = staticMat<double, 3, 3>;
