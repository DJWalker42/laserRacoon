...
namespace phys{
	...
	public:
		...
		RNG(	inputType first,
				inputType second,
				unsigned s = unsigned( std::chrono::system_clock::now().time_since_epoch().count()) ):
				engine(s),
				dist(first,second){}
		...
		/*	Member functions	*/
		void set_seed( std::seed_seq s) { engine.seed(s); }
		void set_seed( unsigned s) { engine.seed(s); }
		...
		distType get_dist() const {return dist;}
	...
	template<	class rngType = std::minstd_rand,
				class intType = int	>
	class RNG_binomial  {
	private:		
		rngType engine;
	...
	public:
		...
		void set_params( intType upper, double prob ){
			typename std::binomial_distribution<intType>::param_type pt(upper, prob);
			dist.param(pt);
		}
	...
	public:
		...
		void set_params( intType upper, double prob ){
			typename std::negative_binomial_distribution<intType>::param_type pt(upper, prob);
			dist.param(pt);
		}
