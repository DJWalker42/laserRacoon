...
public:
	...
	virtual double differential_function(double inde_var,
			const stdVec_d &depend_vars, int num_of_dims, int var_flag)=0;
	void set_order(uint32_t order_to_set) {
		order = order_to_set;
		check_order();
	}
...
public:
	 ...
	 */
	SHM() :
			Diff_eqn(), k(9.0), D(0.0), drive_func() {
	}
	/*	Constructor to set spring constant and drag.
	 */
	SHM(double spring, double drag) :
			Diff_eqn(), k(spring), D(drag), drive_func() {
	}
	/*	Constructor for a user defined drive function */
	SHM(double spring, double drag, double (*d_func)(double)) :
			Diff_eqn(), k(spring), D(drag), drive_func(d_func) {
	}
	...
	void set_spring_const(double new_k) {
		k = new_k;
	}
	void set_drag_coeff(double new_D) {
		D = new_D;
	}
	void set_drive_func(double (*f)(double)) {
		drive_func = f;
	}
	void turn_off_drive() {
		drive_func = 0;
...
public:
	Gravity(double drag = double(), int turb = -1) :
			Diff_eqn(), D(drag), turbulent(turb) {
	}
	double differential_function(double x, const stdVec_d &y, int N, int c) {
		double r { 1. };
		switch (N) {
		case 1:
			...
			break;
		case 2:
			r = phys::mag2D(y[0], y[1]);
			break;
		case 3:
			r = phys::mag3D(y[0], y[1], y[2]);
			break;
		default:
		...
		if (D != double()) {
			...
			if (turbulent)
				vel *= fabs(y[c + N]); //use drag proportional to v squared.
			result -= D * vel; //minus as drag force acts in opposite direction to velocity.
		}
		return result;
	}
	void set_drag_coeff(double drag) {
		D = drag;
	}
	void toggle_turbulence() {
		turbulent = -turbulent;
	}
private:
	...
	void __error_gravity_dims() {
		std::string errmsg =
				"Gravity class only supports 1, 2, or 3 spatial dimensions";
		throw(std::runtime_error(errmsg));
	}
};
...
public:
	...
	Duffing() :
			Diff_eqn(), beta(9.0), alpha(0.0), delta(0.0), gamma(0.0), omega(
					0.0) {
	}
	//----------------------------------------------------------------------------------
	Duffing(double stiffness, double nonLinear, double damping,
			double amplitude, double frequency) :
			Diff_eqn(), beta(stiffness), alpha(nonLinear), delta(damping), gamma(
					amplitude), omega(frequency) {
	}
	double differential_function(double x, const stdVec_d &y, int N, int c) {
		return (gamma * cos(omega * x)
				- (beta * y[c] + alpha * y[c] * y[c] * y[c] + delta * y[c + N]));
	}
...
/*	Use to define your own differential_function using the member prototype.
 *  Use set_order to change to 1 if necessary. Note that only order 1 and 2
 *  ODEs supported. The vector 'params' is available for you to use in your
 *  differential_function; just use set_parameters function in your main.
 */
class User_eqn: public Diff_eqn {
public:
	User_eqn(uint32_t order) :
			Diff_eqn(order), params() {
	}
public:
	 ...
	 */
	double differential_function(double independent, const stdVec_d &dependent,
