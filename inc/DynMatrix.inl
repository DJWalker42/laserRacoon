...
matrix<T>::matrix(size_t m) :
		data_array(m, std::vector < T > (m, 0)), trans(false) {
}
...
matrix<T>::matrix(const std::vector<T> &v, size_t rows, size_t cols, bool is_T) :
		data_array(rows, std::vector < T > (cols)), trans(is_T) {
...
template<class T>
const matrix<T> matrix<T>::operator()(size_t imin, size_t imax, size_t jmin,
		size_t jmax) const {
...
template<typename T>
const matrix<T>& matrix<T>::resize(size_t resize_size,
		const std::vector<T> &val, dimension h_w) {
	switch (h_w) {
	case HEIGHT:
...
template<class T>
const matrix<T> resize(const matrix<T> &m, size_t resize_size,
		const std::vector<T> &val, dimension which) {
	return matrix < T > (m).resize_rows(resize_size, val, which);
}

template<class T>
const matrix<T> pad_matrix(const matrix<T> &m, size_t rows, size_t cols,
		const T &val) {
